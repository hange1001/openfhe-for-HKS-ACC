...
## ADR-014: 微架构在 P4 收口，P5 剩余旁路与 Shoup 延后

- **日期**: 2026-09-05
- **决策**: 以提交 `e948a69` 作为当前固定参数的微架构收尾节点；不实施 P5 剩余
  输出旁路改写，不把 Shoup 接入 Top/OpenFHE ABI。保留独立 Shoup 原语作为工业参数下
  DSP 受限时的候选。
- **原因**:
  - P4 已完成 AXI256、burst、共享工作区、单 scratch、算术复用和 OOC 物理验收；
  - P5 绕开 `result_buffer` 不能删除通用 ADD/SUB/MULT 仍需要的物理存储，并引入
    DDR 重读或输入输出别名风险；
  - Shoup 独立原语可将 BConv 15 路预计减少 330 DSP，但 BConv 已为 II=1，当前参数
    下两个 digit 的速度预测低于0.02%，接入却要求 ABI 版本、元数据和两入口全链修改；
  - 工业参数尚未冻结，继续针对 N4096/Q3/P2 局部优化容易得到不可迁移结论。
- **影响**: P4 源码不变；Shoup WIP 保持不可达；下一阶段先确定工业参数与板级测量，
  再评估 INIT 有效模数装载、BConv 包装器统一、Shoup 和分块/并行。
- **替代方案**:
  - 立即接入 Shoup：资源收益明确但当前速度收益过低，延后；
  - 删除独立 OP_BCONV 以消除一个包装器：会改变兼容接口，延后到 HKS-only kernel 决策；
  - 完成 P5 旁路改写：没有可回收存储且别名风险高，不采用。
- **参考**: `doc/HKS_微架构阶段收尾报告.md`；P4 报告；提交 `e948a69`。
